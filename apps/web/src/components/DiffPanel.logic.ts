...
import { type ScopedThreadRef, type ThreadId, type TurnId } from "@t3tools/contracts";
...
export function useDiffContextCommentDrafts(args: {
  ...
  const { activeThreadId, activeThreadRef, selectedTurnId, renderableFiles } = args;
  ...
  const pendingDiffContextComments = useComposerDraftStore((state) =>
    activeThreadRef
      ? (state.getComposerDraft(activeThreadRef)?.diffContextComments ??
  ...
  }, [activeThreadRef, selectedTurnId]);
  ...
  const submitManualComment = useCallback(() => {
    ...
    addDiffContextComment(activeThreadRef, {
      id: randomUUID(),
  ...
  const saveEditingComment = useCallback(() => {
    if (!activeThreadRef || !editingCommentId) {
      return;
    }
    ...
    updateDiffContextComment(activeThreadRef, editingCommentId, {
      body: commentBody,
    });
    setEditingCommentId(null);
    setEditingCommentBody("");
    setEditingCommentError("");
  }, [activeThreadRef, editingCommentBody, editingCommentId, updateDiffContextComment]);
  ...
  const deleteEditingComment = useCallback(() => {
    ...
    removeDiffContextComment(activeThreadRef, editingCommentId);
    cancelEditingComment();
  }, [activeThreadRef, cancelEditingComment, editingCommentId, removeDiffContextComment]);
